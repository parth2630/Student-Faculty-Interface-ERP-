...
<body>
<h4>Application for Bonafide Certificate:- </h4>
<form method="post" enctype="multipart/form-data">
    <input type="file" name="file">
    <textarea name="comment" placeholder="Enter Application details"></textarea>
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
                ...
                if (!empty($_POST["comment"])) {
                    ...
                    $sql = "INSERT INTO bonafide (name, size, type, path, text) VALUES ('$fileName', '$fileSize', '$fileType', '$targetFile','$comment')";
                    if ($conn->query($sql) === TRUE) {
                        echo "<script type='text/javascript'>alert('Exam details has been updated.');</script>";
                    }} else {
    ...
    if (!empty($_POST["delete_file"])) {
        ...
        $sql = "DELETE FROM bonafide WHERE id='$file_id'";
        if ($conn->query($sql) === TRUE) {
            // Delete the file from the file system
            // Add your code to delete the file here
            echo "<script type='text/javascript'>alert('Exam notice deleted successfully.');</script>";
        } else {
    ...
    if (!empty($_POST["edit_file"])) {
        ...
        $sql = "UPDATE bonafide SET text='$new_comment' WHERE id='$file_id'";
        if ($conn->query($sql) === TRUE) {
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<div>";
        echo "<strong>$count:</strong> " . $row["name"] . "<br>";
