...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
                ...
                if (!empty($_POST["comment"])) {
                    ...
                    $sql = "INSERT INTO exam (name, size, type, path, comment) VALUES ('$fileName', '$fileSize', '$fileType', '$targetFile','$comment')";
    ...
    if (!empty($_POST["delete_file"])) {
        ...
        $sql = "DELETE FROM exam WHERE id='$file_id'";
        if ($conn->query($sql) === TRUE) {
    ...
    if (!empty($_POST["edit_file"])) {
        ...
        $sql = "UPDATE exam SET comment='$new_comment' WHERE id='$file_id'";
        if ($conn->query($sql) === TRUE) {
